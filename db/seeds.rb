...
Post.create(:category_id => lost.id, :body => "My labradoodle Rover has been missing for three days. I am lost without his touch. Please help.", :user_id => shawn.id, :like => 0)
Post.create(:category_id => misc.id, :body => "The Rock is overrated.", :user_id => daniel.id, :like => 0)
Post.create(:category_id => item.id, :body => "Gently used championship belt.", :user_id => hunter.id, :like => 0)

Post.create(:category_id => lost.id, :body => "Missing Butterfree. He responds to the name Dave. You can find me in a tent by the river if you find him.", :user_id => brock.id, :like => 0)
Post.create(:category_id => item.id, :body => "Selling a gold nugget. $5000 OBO; no trades.", :user_id => misty.id, :like => 0)
Post.create(:category_id => misc.id, :body => "I just beat the Elite Four! I'm the very best.", :user_id => ash.id, :like => 0)
Post.create(:category_id => job.id, :body => "Looking for a young apprentice to follow in my footsteps through my scientific endeavors. \nQualifications:\n\nMust love animals\nPreferably male\nYoung at heart\nLittle to no parental supervision\n\n Apply at the Oak Laboratory.", :user_id => oak.id, :like => 0)
Post.create(:category_id => misc.id, :body => "Is anyone looking to trade a Spearow for my Farfetch'd named DUX? I named him when I was 7 years old and now he hates me. Please respond quickly.", :user_id => misty.id, :like => 0)
Post.create(:category_id => lost.id, :body => "Where is my grandson? Are you my grandson?", :user_id => oak.id, :like => 0)
Post.create(:category_id => lost.id, :body => "Where is my grandpa? Did he get lost again? He needs his medicine.", :user_id => gary.id, :like => 0)
Post.create(:category_id => misc.id, :body => "Smell ya later Ash!", :user_id => gary.id, :like => 0)
Post.create(:category_id => job.id, :body => "I need some cheerleaders to follow me around while I train to be the best!", :user_id => gary.id, :like => 0)
Post.create(:category_id => misc.id, :body => "Ash is a loser!!!", :user_id => gary.id, :like => 0)
Post.create(:category_id => misc.id, :body => "I earned so many badges already I'm takin' a break to work on my tan!", :user_id => gary.id, :like => 0)
Post.create(:category_id => misc.id, :body => "I'm here to battle, not to talk. What about you?", :user_id => gary.id, :like => 0)
Post.create(:category_id => misc.id, :body => "Well, Ash, ya snooze ya lose, and you're behind right from the start!", :user_id => gary.id, :like => 0)

Post.create(:category_id => misc.id, :body => "How does this work? Patrick are you there?", :user_id => spongebob.id, :like => 0)
Post.create(:category_id => misc.id, :body => "I'm here Spongebob! This is great!", :user_id => patrick.id, :like => 0)
Post.create(:category_id => lost.id, :body => "Has anyone seen Wormy? I've looked all over for him and he hasn't turned up.", :user_id => sandy.id, :like => 0)
Post.create(:category_id => job.id, :body => "Looking for a new line cook to join the Krusty Krab! Payment will be discussed on acceptance.", :user_id => krabs.id, :like => 0)
Post.create(:category_id => item.id, :body => "Selling a lightly used clarinet. Serious replies only.", :user_id => squidward.id, :like => 0)
